...
void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node) {
  ...
  node_->declare_parameter("fsm/target_mode", -1);
  ...
  node_->get_parameter("fsm/target_mode", target_mode_);
  ...
  if (planner_manager_->pp_.drone_id >= 1) {
    ...
    swarm_trajs_sub_ =
        node_->create_subscription<traj_utils::msg::MultiBsplines>(
            sub_topic_name, 10,
            [this](const std::shared_ptr<const traj_utils::msg::MultiBsplines>
                       &msg) { this->swarmTrajsCallback(msg); });
  }
  ...
  if (planner_manager_->pp_.drone_id <= -1) {
    RCLCPP_INFO(node_->get_logger(), "single drone:%d",
                planner_manager_->pp_.drone_id);
    pub_topic_name =
        string("/drone_") + "single" + string("_planning/swarm_trajs");
  } else {
    pub_topic_name = string("/drone_") +
                     std::to_string(planner_manager_->pp_.drone_id) +
                     string("_planning/swarm_trajs");
  }

  swarm_trajs_pub_ = node_->create_publisher<traj_utils::msg::MultiBsplines>(
      pub_topic_name, 10);
  ...
  broadcast_bspline_sub_ = node_->create_subscription<traj_utils::msg::Bspline>(
      "planning/broadcast_bspline_to_planner", 100,
      [this](const std::shared_ptr<const traj_utils::msg::Bspline> &msg) {
        this->BroadcastBsplineCallback(msg);
      });

  bspline_pub_ =
      node_->create_publisher<traj_utils::msg::Bspline>("planning/bspline", 10);
  data_disp_pub_ = node_->create_publisher<traj_utils::msg::DataDisp>(
      "planning/data_display", 100);

  // Always subscribe to both; whichever arrives last sets the current mode.
  single_target_sub_ =
      node_->create_subscription<geometry_msgs::msg::PoseStamped>(
          "/ego_planner/single_target", 1,
          [this](const std::shared_ptr<const geometry_msgs::msg::PoseStamped>
                     &msg) { this->onSingleTarget(msg); });

  waypoint_trigger_sub_ =
      node_->create_subscription<geometry_msgs::msg::PoseArray>(
          "/ego_planner/waypoints_target", 1,
          [this](
              const std::shared_ptr<const geometry_msgs::msg::PoseArray> &msg) {
            this->onWaypointTarget(msg);
          });

  if (target_mode_ == TARGET_TYPE::SINGLE_TARGET) {
    RCLCPP_INFO(node_->get_logger(),
                "Single-target mode: waiting for /ego_planner/single_target");
  } else if (target_mode_ == TARGET_TYPE::WAYPOINT_TARGET) {
    RCLCPP_WARN(node_->get_logger(), "Waypoint mode: waiting for PoseArray on "
                                     "/ego_planner/waypoints_target");
  } else {
    RCLCPP_WARN(
        node_->get_logger(),
        "Unknown flight_type '%d'; defaulting to single-target behavior",
        target_mode_);
...
void EGOReplanFSM::onWaypointTarget(
    const std::shared_ptr<const geometry_msgs::msg::PoseArray> &msg) {
  waypoints_.clear();
  next_wp_index_ = 0;

  for (const auto &pose : msg->poses) {
    waypoints_.emplace_back(pose.position.x, pose.position.y, pose.position.z);
  }

  if (waypoints_.empty()) {
    RCLCPP_WARN(node_->get_logger(),
                "Waypoint trigger received with empty list. Ignoring.");
    return;
  }

  target_mode_ = TARGET_TYPE::WAYPOINT_TARGET;
  waypoint_triggered_ = true;
  cout << "Waypoint route received! Loaded " << waypoints_.size()
       << " waypoints." << endl;
  init_pt_ = odom_pos_;

  if (!have_odom_) {
    RCLCPP_WARN(node_->get_logger(),
                "Waypoint route received but odom not ready yet. Will wait for "
                "odom before planning.");
    return;
  }

  // Plan toward the first waypoint in the list.
  planNextWaypoint(waypoints_.front());
}

void EGOReplanFSM::onSingleTarget(
  ...
  if (!have_odom_) {
    RCLCPP_WARN(node_->get_logger(),
                "Single target received but odom not ready yet.");
    return;
  }

  cout << "Single target received!" << endl;
  ...
  visualization_->displayGoalPoint(end_wp, Eigen::Vector4d(1.0, 0.4, 0.0, 1.0),
                                   0.3, goal_vis_id++);

  planNextWaypoint(end_wp);
}

void EGOReplanFSM::odometryCallback(
  ...
  if (target_mode_ == TARGET_TYPE::SINGLE_TARGET && !have_target_) {
    // In single-target mode, odom readiness alone doesn't trigger planning; we
    // wait for a goal message. This branch is intentional to avoid unintended
    // replans.
  }
}
...
void EGOReplanFSM::swarmTrajsCallback(
  ...
  if (!have_odom_) {
    RCLCPP_ERROR(node_->get_logger(),
                 "swarmTrajsCallback(): no odom!, return.");
    return;
  }
...
void EGOReplanFSM::execFSMCallback() {
  ...
  if (fsm_num == 100) {
    ...
    if (!have_odom_)
      cout << "no odom." << endl;
    if (!have_target_)
      cout << "wait for target or waypoint trigger." << endl;
    fsm_num = 0;
  }
  ...
  case WAIT_TARGET: {
    const bool ready = (target_mode_ == TARGET_TYPE::WAYPOINT_TARGET)
                           ? (have_target_ && waypoint_triggered_)
                           : have_target_;
    if (!ready)
      goto force_return;
    else {
  ...
  case SEQUENTIAL_START: // for swarm
  {
    if (planner_manager_->pp_.drone_id <= 0 ||
      ...
      bool trigger_ready = (target_mode_ == TARGET_TYPE::WAYPOINT_TARGET)
                               ? waypoint_triggered_
                               : true;
      if (have_odom_ && have_target_ && trigger_ready) {
        ...
        if (success) {
          ...
          RCLCPP_ERROR(node_->get_logger(),
                       "Failed to generate the first trajectory!!!");
          changeFSMExecState(SEQUENTIAL_START, "FSM");
        }
      } else {
        RCLCPP_ERROR(node_->get_logger(),
                     "No odom or no target! have_odom_=%d, have_target_=%d",
  ...
  case EXEC_TRAJ: {
    ...
    if ((target_mode_ == TARGET_TYPE::WAYPOINT_TARGET) &&
        (next_wp_index_ < static_cast<int>(waypoints_.size())) &&
        (end_pt_ - pos).norm() < no_replan_thresh_) {
      // Move to next waypoint if available.
      next_wp_index_++;
      if (next_wp_index_ < static_cast<int>(waypoints_.size())) {
        planNextWaypoint(waypoints_[next_wp_index_]);
      } else {
  ...
  for (double t = t_cur; t < info->duration_; t += time_step) {
    ...
    if (occ) {
      ...
      if (planFromCurrentTraj()) // Make a chance
      {
        ...
        {
          RCLCPP_WARN(node_->get_logger(),
                      "Suddenly discovered obstacles. emergency stop! time=%f",
                      t - t_cur);

          changeFSMExecState(EMERGENCY_STOP, "SAFETY");
        } else {
          RCLCPP_WARN(node_->get_logger(),
                      "current traj in collision, replan.");
